    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans pb-16">
      ...
      <div className="bg-white border-b border-slate-200 pt-4 pb-4 px-4 sm:px-8 lg:px-16">
          ...
          {hero.stats && (
            <div className="hidden lg:flex items-center gap-2 bg-indigo-50/50 px-3 py-2 rounded-xl border border-indigo-100/50">
              ...
              <div className="text-xs text-indigo-900 font-medium">
                <span>Over {hero.stats.count} {hero.stats.label}</span>
                <div className="text-slate-500 font-normal">{hero.stats.sublabel}</div>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="max-w-5xl mx-auto mt-3 bg-white rounded-xl shadow-sm border border-slate-200 p-1.5 grid grid-cols-1 md:grid-cols-12 gap-1.5">
          ...
          <div className="md:col-span-2 flex items-center px-2.5 py-1.5 bg-slate-50 rounded-lg border border-transparent focus-within:border-blue-500 focus-within:bg-white transition cursor-pointer justify-between">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 lg:px-16 mt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-3 bg-white p-6 rounded-2xl border border-slate-200 h-fit space-y-6">
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2 text-sm text-slate-600">
              {['All Locations', 'Remote', 'Bangalore', 'Delhi', 'Mumbai', 'Pune'].map((loc, idx) => (
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2 text-sm text-slate-600">
              {['All Categories', 'Frontend', 'Backend', 'Full Stack', 'DevOps', 'UI/UX', 'Data Science'].map((cat, idx) => (
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              <input type="text" placeholder="Min Salary" className="bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-xs text-slate-700 outline-none focus:border-blue-500" />
              <input type="text" placeholder="Max Salary" className="bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-xs text-slate-700 outline-none focus:border-blue-500" />
            </div>
          </div>

          <button className="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-2.5 rounded-xl text-sm transition shadow-sm">
            Apply Filters
          </button>
        </div>
        ...
        <div className="lg:col-span-9 space-y-4">
          ...
          <div className="flex items-center justify-between px-1">
            <span className="text-sm text-slate-500 font-medium">Showing 1-9 of 120 jobs</span>
            <div className="flex items-center gap-2 text-sm">
              <span className="text-slate-500">Sort By:</span>
              <div className="flex items-center gap-1 bg-white border border-slate-200 px-3 py-1.5 rounded-lg text-slate-700 font-medium cursor-pointer shadow-sm">
          ...
          {jobs.map((job) => {
            ...
            return (
              <Link key={job.id} href={`/job/${job.slug}`} className="block bg-white p-5 rounded-2xl border border-slate-200 hover:shadow-md transition">
                <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                  <div className="flex items-start gap-4 flex-1">
                    ...
                    <div>
                      <h2 className="font-bold text-slate-900 text-base hover:text-blue-600 transition">{job.title}</h2>
                      <p className="text-slate-500 text-sm">{job.company}</p>
                      
                      <div className="flex flex-wrap items-center gap-2 mt-3">
                        {job.tags.map((tag, idx) => {
                          let badgeStyle = "bg-slate-100 text-slate-600";
                          if (tag.type === 'employment') badgeStyle = "bg-emerald-50 text-emerald-600 border border-emerald-100";
                          if (tag.type === 'mode') badgeStyle = "bg-sky-50 text-sky-600 border border-sky-100";
                          if (tag.type === 'category') badgeStyle = "bg-purple-50 text-purple-600 border border-purple-100";
                          
                          return (
                            <span key={idx} className={`text-xs px-2.5 py-1 rounded-md font-medium ${badgeStyle}`}>
                              {tag.label}
                            </span>
                          );
                        })}
          ...
          <div className="bg-white p-12 rounded-2xl border border-slate-200 text-center flex flex-col items-center justify-center space-y-3">
            ...
            <div>
              ...
              <p className="text-slate-500 text-sm mt-1">Try adjusting your search or filter to find what you're looking for.</p>
            </div>
            <button className="mt-2 px-4 py-2 border border-slate-200 hover:bg-slate-50 rounded-xl text-xs font-semibold text-slate-700 transition">
              Clear Filters
            </button>
          </div>
