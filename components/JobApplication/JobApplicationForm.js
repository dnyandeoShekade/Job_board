  ...
  return (
    <div className="bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-sm">
      ...
      <p className="text-slate-500 text-sm mb-6">
        Fill in the details below to submit your application for {applicationData.title} position.
      </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {!resumeFile ? (
              <label className="flex items-center justify-center w-full px-4 py-8 bg-slate-50 border-2 border-dashed border-slate-300 rounded-lg cursor-pointer hover:bg-slate-100 transition">
                <div className="text-center">
                  ...
                  <span className="text-sm text-slate-600">Click to upload resume</span>
                  <span className="text-xs text-slate-400 block mt-1">PDF only, max {applicationForm.maxFileSize}</span>
                </div>
                <input
                  type="file"
        ...
        >
          {isSubmitting ? applicationSettings.loadingText : applicationSettings.submitButtonText}
        </button>
