...
const STYLES = {
  ...
  yellow: { bg: 'bg-yellow-50', icon: 'text-yellow-500', value: 'text-yellow-600' },
  red:    { bg: 'bg-red-50',    icon: 'text-red-400',    value: 'text-red-500'    },
};
      ...
      {stats.map((stat) => {
        ...
        return (
          <div key={stat.label} className="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm">
            ...
            <p className={`text-2xl font-bold ${s.value}`}>{stat.value}</p>
